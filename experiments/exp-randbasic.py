# Script to run experiments using randbasic strategy
import copy
import random
...
from probesim.strategy import randbasic as strategy
...
def num_probing_paths(Pdict):
    """
    Compute the number of probing paths from a probing dictionary
    """
    tot = 0
    for s in Pdict.keys():
        tot += len(Pdict[s].keys())
    return tot


def randbasic_trial_from_paths(rb_args):
    """
    Run a single randbasic trial when given the graph and probing paths.
    ...
    """
    (ofname, header_row, trial_num, trial_seed, maxs, P, G, p, output, statImp, statTol) = rb_args
...
def randbasic_gnm_trial(rb_args):
    """
    Run a single randbasic trial for specified G_{n,m} parameters
    ...
    """
    (ofname, header_row, trial_num, b, n, m, trial_seed, maxs, k, output, edge_attr, node_attr, statImp, statTol) = rb_args
    ...
    # Compute p so that we expect to probe k paths per timestep
...
def randbasic_ba_trial(rb_args):
    """
    Run a single randbasic trial for specified B--A graph parameters
    ...
    """
    (fractional, ofname, header_row, trial_num, b, n, bam, trial_seed, maxs, k, output, edge_attr, node_attr, statImp, statTol) = rb_args
...
def randbasic_trial_from_beacons(rb_args):
    """
    Run a single randbasic trial when given the graph and beacon set
    ...
    """
    (ofname, header_row, trial_num, G, B, trial_seed, maxs, k, output, edge_attr, node_attr, statImp, statTol) = rb_args
...
# Set up the parameters that are specific to different topology collections
# and classes.

if args.zoo:
    topology_list = glob.glob(args.simroot + 'zoo/*.graphml')
    ofname = 'zoo-randbasic'
    header_row = ['topology', 'trialnum', 'status', 'k', 'p', 'maxS', 'edgeAttr', 'nodeAttr', 'statImp', 'statTol', 'simtime', 'trialseed', 'gseed', 'bseed', 'pseed', 'wseed', 'tseed', 'simseed'] + stats_columns
...
#     top_param_list = [(1,2),(1,3),(2,2),(3,2),(4,2)] # This should be an iterable of (dcn,dck) tuples
    if args.dcell:
        ofname = 'dcell-randbasic'
    else:
        ofname = 'spdcell-randbasic'
    header_row = ['dcn', 'dck', 'trialnum', 'status', 'k', 'p', 'maxS', 'edgeAttr', 'nodeAttr', 'statImp', 'statTol', 'simtime', 'trialseed', 'gseed', 'bseed', 'pseed', 'wseed', 'tseed', 'simseed'] + stats_columns
...
#     top_param_list = [(6,12,12), (42,84,105), (42,63,84), (156,208,312), (2,4,3), (9,25,48), (16,36,80), (16,46,120), (27,105,234), (6,9,9), (12,16,18), (16,48,128), (64,256,768), (64,190,504), (20,25,30)] # This should be an iterable of (b,n,m) tuples
    ofname = 'gnm-randbasic'
    header_row = ['b', 'n', 'm', 'trialnum', 'status', 'k', 'p', 'maxS', 'edgeAttr', 'nodeAttr', 'statImp', 'statTol', 'simtime', 'trialseed', 'gseed', 'bseed', 'pseed', 'wseed', 'tseed', 'simseed'] + stats_columns
    if args.test or args.local:
        top_param_list = top_param_list[:3]
...
#     top_param_list = [(2,[3,3],[4,1]),(2,[4,4],[6,1]),(3,[3,3,3],[6,1,1]),(2,[4,4],[4,4]),(3,[4,4,4],[4,4,4]),(3,[4,4,4],[6,1,1])]
    ofname = 'xgft-randbasic'
    header_row = ['h', 'mlist', 'wlist', 'trialnum', 'status', 'k', 'p', 'maxS', 'edgeAttr', 'nodeAttr', 'statImp', 'statTol', 'simtime', 'trialseed', 'gseed', 'bseed', 'pseed', 'wseed', 'tseed', 'simseed'] + stats_columns
    if args.test or args.local:
        top_param_list = top_param_list[:3]
...
#     top_param_list = [(2,4,3)] # This should be an iterable of (b,n,m) tuples
    ...
    ofname = 'ba-randbasic'
    header_row = ['fractional', 'b', 'n', 'bam', 'trialnum', 'status', 'k', 'p', 'maxS', 'edgeAttr', 'nodeAttr', 'statImp', 'statTol', 'simtime', 'trialseed', 'gseed', 'bseed', 'pseed', 'wseed', 'tseed', 'simseed'] + stats_columns
    if args.test or args.local:
...
#     top_param_list = [(6,12,1.10102), (42,84,1.26918), (42,63,1.36281), (156,208,1.51098), (2,4,1.0), (9,25,2.09567), (16,36,2.3795), (16,46,2.77625), (27,105,2.27799), (6,9,1.1459), (12,16,1.21767), (16,48,2.83399), (64,256,3.03601), (64,190,2.69074), (20,25,1.2639)] # This should be an iterable of (b,n,m) tuples
    ...
    ofname = 'smallcat-randbasic'
    header_row = ['IID', 'trialnum', 'status', 'k', 'p', 'maxS', 'edgeAttr', 'nodeAttr', 'statImp', 'statTol', 'simtime', 'trialseed', 'gseed', 'bseed', 'pseed', 'wseed', 'tseed', 'simseed'] + stats_columns
    if args.test or args.local:
...
for k in kList:
    ...
    if args.zoo:
        ...
        for topofile in topology_list:
            ...
            output = {'topology': topofile, 'k':k, 'maxS':maxs, 'edgeAttr':edge_attr_str, 'nodeAttr':node_attr_str, 'statImp':statImpStr, 'statTol':statTolStr}

            # Set up a generator that produces arguments to the trial
            # function.  Call pool.map to apply the trial function to these
            # arguments
            trial_arg_gen = ((ofile, header_row, i, G, num_beacons, control_RNG.randint(-sys.maxint - 1, sys.maxint), maxs, k, copy.deepcopy(output), edge_attr, node_attr, statImp, statTol) for i in range(number_of_trials))
            
            pool.map(randbasic_trial_from_num_beacons, trial_arg_gen)
        ...
        for j in range(len(IID_list)):
            ...
            pool.map(randbasic_trial_from_paths, trial_arg_gen)
        ...
        for (dcn,dck) in top_param_list:
            ...
            output = {'dcn':dcn, 'dck':dck, 'p':p, 'k':k, 'maxS':maxs, 'edgeAttr':edge_attr_str, 'nodeAttr':node_attr_str, 'statImp':statImpStr, 'statTol':statTolStr}
        ...
        for (dcn,dck) in top_param_list:
            ...
            output = {'dcn':dcn, 'dck':dck, 'k':k, 'maxS':maxs, 'edgeAttr':edge_attr_str, 'nodeAttr':node_attr_str, 'statImp':statImpStr, 'statTol':statTolStr}

            trial_arg_gen = ((ofile, header_row, i, G, B, control_RNG.randint(-sys.maxint - 1, sys.maxint), maxs, k, copy.deepcopy(output), edge_attr, node_attr, statImp, statTol) for i in range(number_of_trials))
            
            pool.map(randbasic_trial_from_beacons, trial_arg_gen)
        ...
        for (h, mList, wList) in top_param_list:
            ...
            output = {'h':h, 'mlist':mList, 'wlist':wList, 'k':k, 'maxS':maxs, 'edgeAttr':edge_attr_str, 'nodeAttr':node_attr_str, 'statImp':statImpStr, 'statTol':statTolStr}
        ...
        for (b,n,m) in top_param_list:
            ...
            output = {'b':b, 'n':n, 'm':m, 'k':k, 'maxS':maxs, 'edgeAttr':edge_attr_str, 'nodeAttr':node_attr_str, 'statImp':statImpStr, 'statTol':statTolStr}
            ...
            if args.seedfile:
                ...
                gnm_trial_arg_gen = ((ofile, header_row, i, b, n, m, seed_list[i], maxs, k, copy.deepcopy(output), edge_attr, node_attr, statImp, statTol) for i in range(number_of_trials))  
            else:
                gnm_trial_arg_gen = ((ofile, header_row, i, b, n, m, control_RNG.randint(-sys.maxint - 1, sys.maxint), maxs, k, copy.deepcopy(output), edge_attr, node_attr, statImp, statTol) for i in range(number_of_trials))
            
            pool.map(randbasic_gnm_trial, gnm_trial_arg_gen)
        ...
        for (b,n,bam) in top_param_list:
            ...
            output = {'b':b, 'n':n, 'bam':bam, 'k':k, 'maxS':maxs, 'edgeAttr':edge_attr_str, 'nodeAttr':node_attr_str, 'statImp':statImpStr, 'statTol':statTolStr}



            # Now produce argument generator for randbasic_ba_trial.  The first
            # field in the tuple indicates whether to use the fractional
            # version.
            if args.ba:
                ba_trial_arg_gen = ((False, ofile, header_row, i, b, n, bam, control_RNG.randint(-sys.maxint - 1, sys.maxint), maxs, k, copy.deepcopy(output), edge_attr, node_attr, statImp, statTol) for i in range(number_of_trials))
            elif args.fracba:
                ba_trial_arg_gen = ((True, ofile, header_row, i, b, n, bam, control_RNG.randint(-sys.maxint - 1, sys.maxint), maxs, k, copy.deepcopy(output), edge_attr, node_attr, statImp, statTol) for i in range(number_of_trials))

            pool.map(randbasic_ba_trial, ba_trial_arg_gen)
